...
func (ext *External) GetProfile(ctx context.Context, token string) (Profile, error) {
	...
	if err != nil {
		helpers.Logger.Error("failed request to ums")
		return Profile{}, errors.Wrap(err, "failed to call http request to ums")
	}
	...
	if err != nil {
		return response, errors.Wrap(err, "failed to create decode response")
	}
